"""Audio extraction from URLs (via yt-dlp) and uploaded files (via FFmpeg)."""
...
async def extract_audio_from_url(url: str) -> dict:
    ...
    cmd = [
        yt_dlp_executable, "-x", "--audio-format", "wav",
        "--audio-quality", "0", "--no-playlist",
        "--socket-timeout", "30", "--max-filesize", "100m",
        "-o", output_path, url
    ]
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            if "Private video" in error_output:
                raise ValueError("This video is private. Only public videos can be summarized.")
            elif "Video unavailable" in error_output:
                raise ValueError("This video is unavailable. It may have been deleted or is geo-restricted.")
            elif "Unsupported URL" in error_output:
                raise ValueError("This URL is not supported. Please provide a valid YouTube or supported video URL.")
                ...
                raise ValueError(f"Failed to download video audio. Error: {error_output[:200]}")
        ...
        return {"audio_path": wav_path, "source": "url", "original_url": url}

    except asyncio.TimeoutError:
        raise ValueError("Video download timed out (5 min limit). Try a shorter video or upload directly.")
    except ValueError:
        raise
...
async def extract_audio_from_file(file_path: str) -> dict:
    ...
    return {"audio_path": wav_path, "source": "upload"}
...
async def _convert_to_whisper_format(input_path: str, file_id: str) -> str:
    ...
    cmd = [ffmpeg_executable, "-i", input_path, "-ar", "16000", "-ac", "1", "-y", output_path]
    ...
    try:
        ...
        logger.info(f"Audio converted: {output_path} ({file_size / 1024 / 1024:.1f} MB)")
        return output_path

    except asyncio.TimeoutError:
        raise ValueError("Audio conversion timed out. The file may be too large or corrupted.")
    except ValueError:
        raise
    except Exception as e:
